...
<%
	...
	ResultSet resultSet = enterprise.getBriefOngoingOrderDetail();
%>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
        ...
        <nav class = "navbar">
            <ul>
                ...
                <li  class = "hidden-desktop initial"><a href = "#">user +</a>
                    <ul>
                        <li><a href = "employeeprofile.jsp">account</a></li>
                        <li><a href = "#"><%=employee.getEmail() %></a></li>
                        <li><a href = "#"><%=employee.getRole() %></a></li>
                    </ul>
                </li>
    ...
    <div class="passworFromDiv" id="passwordFormDiv">
      <form id="passwordForm" action="http://localhost:8080/JavaServerPagesFirst/OrderController" method="get">
        <label id="password-label" for="password">Let the customer input password</label><br>
        <input type="password" id="password" name="password" required style="height: 40px; margin-left: 0;"/>
      ...
      <table id="myTable" class="display">
        ...
        <tbody>
          ...
          <tr data-id="<%= i %>">
            ...
            String status = resultSet.getString("delivery_status").equals("delivered")? "Delivered": "Verify Delivery";
            %>
              <button class="view-detail" id="showFormBtn" onclick='showPasswordForm(<%=resultSet.getString("order_id")%>, <%=customer%>)'>
				<%=status%>
			  </button>
            </td>
            <td>
...
<script>
    ...
    if (checkPassword != null) {
        if (checkPassword.equals("right")) {
            session.setAttribute("checkpassword", ""); // Reset the session attribute
%>
            <script>
                alert("Delivery verification successful");
            </script>
<%
        } else if (checkPassword.equals("wrong")) {
            session.setAttribute("checkpassword", ""); // Reset the session attribute
%>
            <script>
                alert("Wrong customer password. Try again!");
            </script>
<%
        }
    }
